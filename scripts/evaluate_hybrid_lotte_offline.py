...
def load_qrels_from_report():
    qrels_by_query_id = {}
    report_path = EVALUATIONS_DIR / "embedding" / "evaluation_report.json"
    if not report_path.exists():
        print(f"Qrels source report not found: {report_path}")
    ...
    for entry in report["per_query_results"]:
        ...
        qrels_by_query_id[qid] = {str(did): 1.0 for did in relevant_ids}
    return qrels_by_query_id


def evaluate_parallel(hybrid_service, query_text, relevance_by_doc_id, config, k):
...
def evaluate_serial(hybrid_service, query_text, relevance_by_doc_id, config, k):
...
def evaluate_config(hybrid_service, queries, qrels_by_query_id, config, k=10, max_queries=None):
    print(f"\n{'='*60}")
    ...
    for qid in sorted(queries.keys()):
        ...
        if config["mode"] == "parallel":
            result = evaluate_parallel(
                hybrid_service, query_text, relevance_by_doc_id, config, k,
            )
        else:
        ...
        ndcg = ndcg_at_k(retrieved, relevance_by_doc_id, k)
...
def main():
    print("Loading queries from processed_data...")
    queries = load_queries()
    print(f"Queries loaded: {len(queries):,}")

    print("Loading QRELS from embedding evaluation report...")
    qrels_by_query_id = load_qrels_from_report()
    print(f"Queries with QRELS: {len(qrels_by_query_id):,}")
    ...
    for config in HYBRID_CONFIGS:
        report = evaluate_config(
            hybrid_service, queries, qrels_by_query_id, config,
